...
def create_line_chart(
    ...
    import plotly.express as px
    df = pd.DataFrame(data)
    fig = px.line(df, x=x_col, y=y_col, color=color_col if color_col else None, title=title)
    #fig.show()
    return {"fig": fig.to_json(), "title": title}
...
def create_plot_agent(llm, tools):
    ...
    You MUST use the question the user asks to infer human readale and meaningful column names to use when plotting for axis labels, legends, ect.
    """
